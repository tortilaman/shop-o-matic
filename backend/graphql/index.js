...
// Modules
import { Food, Product, Brand, Store, ListItem } from "../db";
...
const ListItemTC = composeWithMongoose(ListItem, customizationOptions);
...
// Associate a favorite product with each food (if it exists)
FoodTC.addRelation("favorite", {
...
schemaComposer.Query.addFields({
  shoppingListById: ListItemTC.getResolver("findById"),
  shoppingListByIds: ListItemTC.getResolver("findByIds"),
  shoppingListOne: ListItemTC.getResolver("findOne"),
  shoppingListMany: ListItemTC.getResolver("findMany"),
  shoppingListCount: ListItemTC.getResolver("count"),
  shoppingListConnection: ListItemTC.getResolver("connection"),
  shoppingListPagination: ListItemTC.getResolver("pagination"),
});

schemaComposer.Mutation.addFields({
  shoppingListCreateOne: ListItemTC.getResolver("createOne"),
  shoppingListCreateMany: ListItemTC.getResolver("createMany"),
  shoppingListUpdateById: ListItemTC.getResolver("updateById"),
  shoppingListUpdateOne: ListItemTC.getResolver("updateOne"),
  shoppingListUpdateMany: ListItemTC.getResolver("updateMany"),
  shoppingListRemoveById: ListItemTC.getResolver("removeById"),
  shoppingListRemoveOne: ListItemTC.getResolver("removeOne"),
  shoppingListRemoveMany: ListItemTC.getResolver("removeMany"),
});

// Product should return name of food, not its ID
ListItemTC.addRelation("item", {
  resolver: () => FoodTC.getResolver("findById"),
  prepareArgs: {
